    ...
    async searchWikipedia(req: Request, res: Response, next: NextFunction): Promise<void> {
        try {
            ...
            if (!searchResponse.data.query.search.length) {
                res.status(404).json({ message: 'Không tìm thấy thông tin về tác phẩm này' });
                return;
            }
            ...
            res.status(200).json({
                ...
                fullContent: fullContent // Thêm nội dung đầy đủ
            });
        } catch (error) {
            console.error('Error fetching Wikipedia data:', error);
            res.status(500).json({ message: 'Lỗi khi lấy dữ liệu từ Wikipedia' });
        }
    }
}
